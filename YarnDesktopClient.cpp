...
std::string getToken(std::string username, std::string password, std::string serverurl)
{
    CURL* curl;
    CURLcode res;
    curl = curl_easy_init();
    ...
    if (curl)
    {
        ...
        std::string finalRequest = "{\"username\":\"" + username + "\",\"password\":\"" + password + "\"}";
...
void downloadAvatar(std::string avatarUrl, std::string filename)
{
    ...
    if (curl)
    {
        ...
        {
            cout << "Successfully grabbed avatar! stored at: " << filename << imgresult;
        }
...
void button_reply_clicked (GtkLabel *lbl, std::string subject, vector<string> mentions)
{
    ...
    if (twtsArray.IsArray())
    {
        for (SizeType index = 0; index < twtsArray.Size(); index++)
        {
            if (twtsArray[index]["markdownText"] != NULL)
            {
                ...
                //Do not include yourself in the reply if you hit reply on your own post.
                if ((post->authorUri.find(userinfo->username) != string::npos) == false)
                {
                ...
                if (twtsArray[index]["mentions"] != NULL)
                {
                    ...
                    {
                        // TODO: remove 'yourself' from the mentions, so that it only appends others - not yourself.
                        for (SizeType mentionIndex = 0; mentionIndex < mentionsArray.Size(); mentionIndex++)
                        {
                            //Check if the 'mentions' contains userinfo.username - skip if it has the 'owners' name in it.
                            std::string tmpString = document["twts"][index]["mentions"][mentionIndex].GetString();
                            
                            if ((tmpString.find(userinfo->username) != string::npos) == false)
                            {
                            ...
                            std::cout << std::endl << "replystring:" << finalReplyString << std::endl;
                        }
                    }
                }
                ...
                //We do not check file format for images, so we just store it with png as extension - regardless of format right now.
                ...
                std::cout << std::endl << "finalTwtString:" << finalTwtString << std::endl;
                ...
                gtk_grid_attach(GTK_GRID(timelineGrid), statusLabel, 1, index + guiLineOffset, 1, 1);
                
                GtkWidget *avatar = gtk_image_new_from_file(filename.c_str());
                gtk_image_set_pixel_size(GTK_IMAGE(avatar), 50);
                gtk_grid_attach(GTK_GRID(timelineGrid), avatar, 0, index + guiLineOffset, 1, 1);
                
                //Create a reply button and send the 'subject' with it, so that the new post replies to that status.
                GtkWidget *replyButton = gtk_button_new_with_label ("Reply");

                g_signal_connect(replyButton, "clicked", G_CALLBACK(button_reply_clicked), &post->subject);

                gtk_grid_attach(GTK_GRID(timelineGrid), replyButton, 0, index + guiLineOffset + 1, 1, 1);
                
                GtkWidget *separator1 = gtk_separator_new(GTK_ORIENTATION_HORIZONTAL);
                GtkWidget *separator2 = gtk_separator_new(GTK_ORIENTATION_HORIZONTAL);
                gtk_grid_attach(GTK_GRID(timelineGrid), separator1, 0,  index + guiLineOffset + 2, 1, 1);
                gtk_grid_attach(GTK_GRID(timelineGrid), separator2, 1, index + guiLineOffset + 2, 1, 1);
...
{
    auto selectedTimeLineIndex = gtk_drop_down_get_selected(GTK_DROP_DOWN(timelineDropDown));
    currentTimelineName = timelineNames[selectedTimeLineIndex];
    std::cout << std::endl << "selected timeline: " << currentTimelineName << std::endl;
    refreshTimeline();
}
...
void button_login_clicked (GtkLabel *lbl)
{
    ...
    if (token != "")
    {
        GtkWidget *statusEntrySeparator = gtk_separator_new(GTK_ORIENTATION_HORIZONTAL);
        gtk_widget_set_vexpand(statusEntrySeparator, true);

        GtkWidget *statusEntrySeparator2 = gtk_separator_new(GTK_ORIENTATION_HORIZONTAL);
        gtk_widget_set_vexpand(statusEntrySeparator, true);

        GtkWidget *statusEntrySeparator3 = gtk_separator_new(GTK_ORIENTATION_HORIZONTAL);
        gtk_widget_set_vexpand(statusEntrySeparator, true);
        ...
        gtk_grid_attach(GTK_GRID(statusEntryGrid), statusEntrySeparator2, 3, 2, 1, 1);
...
void button_refresh_timeline_clicked(GtkLabel *lbl)
{
    refreshTimeline();
}
...
static void activate(GtkApplication *app, gpointer user_data) 
{
    ...
    g_signal_connect (button_login, "clicked", G_CALLBACK (button_login_clicked), NULL);
    ...
    gtk_password_entry_set_show_peek_icon(GTK_PASSWORD_ENTRY(input_password),true);
    ...
    g_signal_connect (button_post_status, "clicked", G_CALLBACK (button_post_status_clicked), NULL);
    
    g_signal_connect (button_refresh_timeline, "clicked", G_CALLBACK (button_refresh_timeline_clicked), NULL);

    gtk_scrolled_window_set_child( GTK_SCROLLED_WINDOW( scrolled_window ),timelineGrid );
